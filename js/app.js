...
class HanziLearningApp {
    ...
    displayCurrentCharacter() {
        ...
        // 初始化HanziWriter
        this.initHanziWriter(character.character);
    }
    ...
    // 初始化HanziWriter
    initHanziWriter(character) {
        const container = document.getElementById('hanzi-writer-container');
        const strokeHint = document.getElementById('strokeHint');
        ...
        this.currentHanziWriter = null;

        try {
            // 创建HanziWriter实例
            this.currentHanziWriter = HanziWriter.create(container, character, {
                width: 300,
                height: 300,
                padding: 20,
            ...
            // 更新提示文字
            strokeHint.textContent = `"${character}" 笔顺动画已准备就绪`;
            strokeHint.classList.add('show');
            
            // 显示练习按钮
            quizBtn.style.display = 'inline-block';

        } catch (error) {
            console.warn(`HanziWriter无法加载字符 "${character}":`, error);
            
            // 降级处理：显示静态汉字
            this.showFallbackCharacter(character, container);
            strokeHint.textContent = `"${character}" 暂无笔顺数据，显示静态字形`;
            strokeHint.classList.add('show');
    ...
    animateStrokes() {
        ...
        if (!this.currentHanziWriter) {
            strokeHint.textContent = '无法播放笔顺动画';
            return;
        }
        ...
        // 更新按钮状态
        animateBtn.textContent = '播放中...';
        animateBtn.classList.add('animating');
        strokeAnimation.classList.add('animating');

        const character = this.currentCharacters[this.currentIndex].character;
        strokeHint.textContent = `正在播放 "${character}" 的笔顺动画...`;

        // 播放笔顺动画
        this.currentHanziWriter.animateCharacter({
            onComplete: () => {
                strokeHint.textContent = '笔顺播放完成！';
                
                setTimeout(() => {
                    this.resetAnimationState();
                }, 1500);
            },
            onStrokeComplete: (strokeNum) => {
                const totalStrokes = this.currentHanziWriter.getCharacterData().strokes.length;
                strokeHint.textContent = `第 ${strokeNum + 1} 笔 / 共 ${totalStrokes} 笔`;
            }
        });
    }
    ...
    startStrokeQuiz() {
        ...
        strokeHint.textContent = `请按正确笔顺书写 "${character}"`;
        
        // 开始笔顺练习模式
        this.currentHanziWriter.quiz({
            onMistake: (strokeData) => {
                strokeHint.textContent = '笔顺错误，请重试';
                console.log('笔顺错误:', strokeData);
            },
            onCorrectStroke: (strokeData) => {
                const totalStrokes = this.currentHanziWriter.getCharacterData().strokes.length;
                strokeHint.textContent = `正确！第 ${strokeData.strokeNum + 1} 笔 / 共 ${totalStrokes} 笔`;
            },
            onComplete: (summaryData) => {
                strokeHint.textContent = `练习完成！总分：${summaryData.totalMistakes === 0 ? '满分' : '还需努力'}`;
    ...
    resetAnimationState() {
        ...
        animateBtn.textContent = '播放笔顺';
        animateBtn.classList.remove('animating');
        strokeAnimation.classList.remove('animating');
        ...
        setTimeout(() => {
            if (this.currentHanziWriter) {
                ...
                strokeHint.textContent = '点击按钮观看笔顺动画';
            }
            strokeHint.classList.remove('show');
        }, 1000);
    ...
    // 降级处理：显示静态汉字
    showFallbackCharacter(character, container) {
